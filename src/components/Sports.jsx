...
const Sports = () => {
  ...
  if (loading) return (
    <div className="flex w-screen h-screen items-center justify-center bg-gradient-to-br from-black via-[#0a0a0a] to-[#2a1e00]">
      <Loader text="Loading Sports Leaderboard" />
    </div>
  )
  if (error) return <div style={{ padding: '2rem', color: 'red' }}>Error: {error}</div>

  const sports = winners.filter((w) => {
    const categoryMatch = String((w.eventCategory || w.category || '')).toLowerCase() === 'sports'
    const published = w.isPublished ?? w.event?.isPublished ?? w.eventPublished ?? false
    return categoryMatch && published
  })
  const groups = sports.reduce((acc, w) => {
  ...
  const sortedHouses = [...houses].sort((a, b) => (b.sportsPoints ?? 0) - (a.sportsPoints ?? 0))
  ...
  return (
    <div className="relative min-h-[100vh] w-full bg-gradient-to-br from-black via-[#0a0a0a] to-[#2a1e00] font-sans overflow-x-hidden">
      ...
      <div className="relative w-full font-sans px-8 pb-8 text-white" style={{ paddingTop: '80px' }}>
        <h1 className="relative z-10 text-4xl md:text-5xl lg:text-6xl font-black uppercase tracking-[0.15em] text-transparent bg-clip-text bg-gradient-to-b from-white via-[rgb(212,176,91)] to-[rgba(212,176,91,0.6)] drop-shadow-[0_10px_20px_rgba(212,176,91,0.3)] text-center mb-10 pb-4" style={{ marginTop: '20px' }}>Sports Leaderboard</h1>
        <br />
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', color: 'var(--text-light)', tableLayout: 'fixed' }}>
              <thead>
                <tr>
                  ...
                  <th style={{ padding: '16px 12px', color: 'var(--primary)', fontSize: '0.95rem', fontWeight: 700, width: '50%', textAlign: 'center', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Sports Points</th>
                </tr>
              </thead>
              <tbody>
                {sortedHouses.map((h, idx) => (
                  <tr key={h.houseId} style={{ background: idx % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <td style={{ padding: '16px 12px', color: 'var(--primary)', fontWeight: 800, fontSize: '1.1rem', textAlign: 'center' }}>{h.sportsPoints ?? 0}</td>
                  </tr>
                ))}
              </tbody>
            </table>
